...
public class AppEngineHttpFetcher extends AbstractHttpFetcher {
	...
	private String encodeParameters(Map<String, String> params) {
		...
		for (Entry<String, String> entry : params.entrySet()) {
			try {
				escapedParams.put(URLEncoder.encode(entry.getKey(), "UTF-8"), URLEncoder.encode(entry.getValue(), "UTF-8"));
	...
	private FetchOptions getFetchOptions(HttpRequestOptions requestOptions) {
		return FetchOptions.Builder.disallowTruncate().doNotFollowRedirects().setDeadline(
				requestOptions.getConnTimeout() / 1000.0);
	}
	...
	private static class AppEngineFetchResponse implements HttpResponse {
		...
		public AppEngineFetchResponse(com.google.appengine.api.urlfetch.HTTPResponse httpResponse, boolean bodySizeExceeded,
				String finalUri) {
