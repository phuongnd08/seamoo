//========================================================================
//A modified version of OpenIdServletFilter to fit to SeaMoo structure and
//make it more like Stackoverflow
//Original OpenIdServletFilter copyright notice is below:
...
public class OpenIdAuthenticationFilter implements Filter {
	...
	public static final String DEFAULT_ERROR_MSG = System.getProperty("openid.servlet_filter.default_error_msg",
			"Your openid could not be resolved.");
	public static final String ID_NOT_FOUND_MSG = System.getProperty("openid.servlet_filter.id_not_found_msg",
			"Your openid does not exist.");
	...
	public void init(FilterConfig config) throws ServletException {
		...
		_relyingParty = (RelyingParty) config.getServletContext().getAttribute(RelyingParty.class.getName());
		...
		if (forwardUriHandlerParam != null) {
			try {
				_forwardHandler = ClassLoaderUtil.newInstance(forwardUriHandlerParam, OpenIdServletFilter.class);
			} catch (Exception e) {
	 ...
	 * Returns true if the user associated with this request is authenticated.
	 */
	public static boolean handle(HttpServletRequest request, HttpServletResponse response, String forwardUri) throws IOException,
			ServletException {
		return handle(request, response, RelyingParty.getInstance(), DEFAULT_FORWARD_URI_HANDLER, forwardUri);
	}

	/**
	 * Returns true if the user associated with this request is authenticated.
	 */
	public static boolean handle(HttpServletRequest request, HttpServletResponse response, RelyingParty relyingParty,
			ForwardUriHandler forwardUriHandler, String forwardUri) throws IOException, ServletException {
		...
		try {
			...
			if (user.isAssociated() && RelyingParty.isAuthResponse(request)) {
				...
				if (relyingParty.verifyAuth(user, request, response)) {
					// authenticated
					// continue to process request - The request will often be a
					// login page, so it will know how to route the request
					// properly
					// response.sendRedirect(request.getRequestURI());
					return true;
			...
			if (request.getQueryString() != null)
				returnTo += "?" + request.getQueryString();// openid provider
			// should redirect
			// to the full url,
			// include any
			// original
