...
public class SiteSettingsSteps extends HtmlUnitSteps {
	...
	public void SetSiteMode(String mode) throws IOException {
		...
		WebRequestSettings settings = new WebRequestSettings(new URL(TestConfig.getServerBase() + "/admin/update-site-settings"),
				HttpMethod.POST);

		settings.setRequestParameters(Arrays.asList(new NameValuePair[] { new NameValuePair("operatingMode", mode) }));
	...
	public void assertCurrentModeIndicator(String indicator) {
		HtmlElement modeIndicator = (HtmlElement) page.getByXPath("//div[@id='content']/div[1]/p[1]/strong").get(0);
		assertEquals(modeIndicator.getTextContent(), indicator);
	}

	@Then("current switch button is \"$buttonCaption\"")
	public void assertCurrentSwitchButton(String buttonCaption) throws IOException {
		HtmlButton changeButton = (HtmlButton) page.getByXPath("//div[@id='content']/div[1]/form[1]/p[1]/button[1]").get(0);
