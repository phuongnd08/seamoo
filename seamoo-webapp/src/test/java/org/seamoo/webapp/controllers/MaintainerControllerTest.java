...
public class MaintainerControllerTest {
	...
	public void maintainQuestionIndexForLeagueShouldPerformOperationWithinSafeTimeLimit() {
		when(questionEventDao.getAllByMinimumTimeStamp(eq(1L), anyLong(), anyInt())).thenAnswer(new Answer<List>() {
			int times = 0;
			...
			public List answer(InvocationOnMock invocation) throws Throwable {
				...
				for (int i = 0; i < count; i++)
					result.add(new QuestionEvent(QuestionEventType.CREATE, questionAutoId));
				return result;
			}
		});
		timeProvider.setCurrentTimeStamp(1);
		controller.refreshLeagueQuestionIndexes(1L);
		verify(questionEventDao, times(2)).getAllByMinimumTimeStamp(eq(1L), anyLong(), anyInt());
	}
	...
	public void maintainQuestionIndexForLeagueChangeTheNumericBagAccordingly() {
		...
		QuestionEvent e1 = new QuestionEvent(QuestionEventType.CREATE, 11L);
		e1.setTimeStamp(120L);
		QuestionEvent e2 = new QuestionEvent(QuestionEventType.DELETE, 9L);
		e2.setTimeStamp(100L);
		List events = Lists.newArrayList(e1, e2);
		when(questionEventDao.getAllByMinimumTimeStamp(eq(1L), anyLong(), anyInt())).thenReturn(events);
		controller.refreshLeagueQuestionIndexes(1L);
